...
output = cv.LoadImage('braillerstuv.jpg')
orig = cv.LoadImage('braillerstuv.jpg')
width, height = cv.GetSize(output)
cv.Resize(output, output,cv.CV_INTER_LINEAR)
cv.Resize(orig, orig,cv.CV_INTER_LINEAR)
